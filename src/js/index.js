...
(async function () {
  ...
  const downloadAllResultsBtn = document.getElementById('download-all');

  downloadAllResultsBtn?.addEventListener('click', () => {
    const values = [...document.querySelectorAll('bs-result')]
      .map(el => el.getAttribute('value'))
      .filter(v => v);
    downloadAllResults(values);
  });
  ...
  async function scan() {
    ...
    try {
      ...
      console.log(barcode?.rawValue ?? '', barcodeValue);
