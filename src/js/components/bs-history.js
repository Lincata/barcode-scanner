import { downloadAllResults } from '../helpers/downloadAllResults.js';
...
class BSHistory extends HTMLElement {
  ...
  async connectedCallback() {
    ...
    this.#downloadHistoryBtn = this.shadowRoot?.getElementById('downloadHistoryBtn');

    this.#renderHistoryList((await getHistory())[1] || []);

    this.#downloadHistoryBtn?.addEventListener('click', this.#handleDownloadHistory);
  ...
  /**
   * Handles the download of
   */
  async #handleDownloadHistory() {
    downloadAllResults((await getHistory())[1] || []);
  }
