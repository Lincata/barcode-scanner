...
`,tB=document.createElement("template");tB.innerHTML=`
  ...
  <footer>
    ...
    <button type="button" id="downloadHistoryBtn">Download history</button>
    <button type="button" id="emptyHistoryBtn">Empty history</button>
  </footer>
`;class tO extends HTMLElement{#O=null;#D=null;#I=null;constructor(){super(),this.shadowRoot||this.attachShadow({mode:"open"}).appendChild(tB.content.cloneNode(!0))}async connectedCallback(){this.#O=this.shadowRoot?.getElementById("historyList"),this.#D=this.shadowRoot?.getElementById("emptyHistoryBtn"),this.#I=this.shadowRoot?.getElementById("downloadHistoryBtn"),this.#$((await tr())[1]||[]),this.#I?.addEventListener("click",this.#P),this.#O?.addEventListener("click",this.#F),this.#D?.addEventListener("click",this.#j)}disconnectedCallback(){this.#O?.removeEventListener("click",this.#F),this.#D?.removeEventListener("click",this.#j)}async add(t){if(!t)return;let e={type:"add",message:"Error adding barcode to history"},[o,i=[]]=await tr();if(o||!Array.isArray(i))return this.#V("bs-history-error",e),o;if(i.find(e=>e===t))return;let r=[...i,t],[s]=await ts(r);return s?(tn.error("Error setting history",s),this.#V("bs-history-error",e),s):(this.#O?.insertBefore(this.#U(t),this.#O.firstElementChild),this.#V("bs-history-success",{type:"add",message:"Barcode added to history"}),null)}async remove(t){if(!t)return;let e={type:"remove",message:"Error removing barcode from history"},[o,i=[]]=await tr();if(o||!Array.isArray(i))return this.#V("bs-history-error",e),o;let r=i.filter(e=>e!==t),[s]=await ts(r);if(s)return tn.error("Error setting history",s),this.#V("bs-history-error",e),s;let a=this.#O?.querySelector(`li[data-value="${t}"]`);return a?.remove(),this.#V("bs-history-success",{type:"remove",message:"Barcode removed from history"}),null}async empty(){let[t]=await ts([]);return t?(tn.error("Error setting history",t),this.#V("bs-history-error",{type:"empty",message:"Error emptying history"}),t):(this.#O?.replaceChildren(),this.#V("bs-history-success",{type:"empty",message:"History emptied successfully"}),null)}#$(t){if(!this.#O)return;this.#O.replaceChildren();let e=document.createDocumentFragment();[...t].reverse().forEach(t=>e.appendChild(this.#U(t))),this.#O.appendChild(e)}#U(t){let e,o=document.createElement("li");o.setAttribute("data-value",t),(e=document.createElement("span")).textContent=t;let i=document.createElement("div");i.className="actions";let r=document.createElement("custom-clipboard-copy"),s=r.shadowRoot?.querySelector("button");r.setAttribute("only-icon",""),r.setAttribute("value",t),s?.setAttribute("aria-label",`Copy to clipboard ${t}`),i.appendChild(r);let a=document.createElement("button");return a.type="button",a.className="delete-action",a.setAttribute("data-action","delete"),a.setAttribute("aria-label",`Remove from history ${t}`),a.innerHTML=`
    ...
    `,i.appendChild(a),o.appendChild(e),o.appendChild(i),o}#F=async t=>{let e=t.target;if(e.closest('[data-action="delete"]')){let t=e.closest("li").dataset.value;this.remove(t)}};#j=async()=>{window.confirm("Empty history? This action cannot be undone.")&&this.empty()};async #P(){tA((await tr())[1]||[])}#V(t,e=null){let o=new CustomEvent(t,{bubbles:!0,composed:!0,detail:e});this.dispatchEvent(o)}static defineCustomElement(t="bs-history"){"undefined"==typeof window||window.customElements.get(t)||window.customElements.define(t,tO)}}tO.defineCustomElement(),async function(){let t=document.querySelector("a-tab-group"),e=document.querySelector("video-capture"),o=document.querySelector("bs-settings"),i=document.querySelector("bs-history"),r=document.getElementById("cameraPanel"),s=r.querySelector(".results"),a=document.getElementById("filePanel").querySelector(".results"),n=document.getElementById("scanInstructions"),l=document.getElementById("scanBtn"),d=document.getElementById("dropzone"),c=document.querySelector("resize-observer"),h=document.getElementById("scanFrame"),u=document.getElementById("torchButton"),m=document.getElementById("globalActions"),p=document.getElementById("historyBtn"),b=document.getElementById("historyDialog"),g=document.getElementById("settingsBtn"),v=document.getElementById("settingsDialog"),f=document.getElementById("settingsForm"),y=document.getElementById("cameraSelect"),w=document.getElementById("download-all");w?.addEventListener("click",()=>{tA([...document.querySelectorAll("bs-result")].map(t=>t.getAttribute("value")).filter(t=>t))});let E=null,A=!0;"function"==typeof HTMLDialogElement&&(m?.removeAttribute("hidden"),b?.removeAttribute("hidden"),v?.removeAttribute("hidden"));let{barcodeReaderError:x}=await tb.setup();if(x){let e=document.getElementById("barcodeReaderError");A=!1,m?.setAttribute("hidden",""),t?.setAttribute("hidden",""),e?.setAttribute("open","");return}let k=await tb.getSupportedFormats(),[,L]=await to(),C=L?.formats||k,S=await tb.create(C);e.addEventListener("video-capture:video-play",q,{once:!0}),e.addEventListener("video-capture:error",function(t){let e=t.detail.error;if("NotFoundError"===e.name)return;let o="NotAllowedError"===e.name?"<strong>Error accessing camera</strong><br>Permission to use webcam was denied or video Autoplay is disabled. Reload the page to give appropriate permissions to webcam.":e.message;r.innerHTML=`
    ...
    `},{once:!0}),tE.defineCustomElement();let _=e?.shadowRoot,z=_?.querySelector("video"),T=_?.querySelector('[part="actions-container"]');async function R(){if(A){tn.info("Scanning..."),n?.removeAttribute("hidden");try{let[,t]=await to(),e=await S.detect(z),o=e?.rawValue??"";if(console.log(e?.rawValue??"",o),!o)throw Error("No barcode detected");if(tl(s,o),t?.addToHistory&&i?.add(o),tu(),!t?.continueScanning){E&&(clearTimeout(E),E=null),l?.removeAttribute("hidden"),h?.setAttribute("hidden",""),T?.setAttribute("hidden","");return}}catch{}A&&(E=setTimeout(()=>R(),1e3))}}async function M(t){if(!t)return;let[,e]=await to(),o=new Image,r=new FileReader;r.onload=r=>{let s=r.target.result;o.onload=async()=>{try{let t=await S.detect(o),r=t?.rawValue??"";if(console.log(t?.rawValue??"",r),!r)throw Error("No barcode detected");tl(a,r),e?.addToHistory&&i?.add(r),tu()}catch(t){tn.error(t),tv("<strong>No barcode detected</strong><br><small>Please try again with a different image.</small>",{variant:"danger",trustDangerousInnerHTML:!0}),tu({success:!1})}},o.src=s,o.alt="Image preview",d.replaceChildren();let n=document.createElement("div");n.className="dropzone-preview";let l=document.createElement("div");l.className="dropzone-preview__image-wrapper";let c=document.createElement("div");c.className="dropzone-preview__file-name",c.textContent=t.name,l.appendChild(o),n.appendChild(l),n.appendChild(c),d.prepend(n)},r.readAsDataURL(t)}async function q(t){h?.removeAttribute("hidden"),tm(t.detail.video,h),R();let o=t.target.getTrackSettings(),i=t.target.getTrackCapabilities(),r=document.getElementById("zoomLevel");if(i?.torch&&(u?.addEventListener("click",N),u?.removeAttribute("hidden"),e.hasAttribute("torch")&&tg({el:u,isTorchOn:!0})),o?.zoom&&i?.zoom){let t=document.getElementById("zoomControls"),s=i?.zoom?.min||0,a=i?.zoom?.max||10,n=o?.zoom||1;t?.addEventListener("click",t=>{let o=t.target.closest('[data-action="zoom-in"]'),i=t.target.closest('[data-action="zoom-out"]');o&&n<a&&(n+=.5),i&&n>s&&(n-=.5),r.textContent=n.toFixed(1),e.zoom=n}),t?.removeAttribute("hidden"),r.textContent=n.toFixed(1)}let s=await tE.getVideoInputDevices();s.forEach((t,e)=>{let o=document.createElement("option");o.value=t.deviceId,o.textContent=t.label||`Camera ${e+1}`,y.appendChild(o)}),s.length>1&&(y?.addEventListener("change",B),y?.removeAttribute("hidden"))}async function H(t){t.preventDefault();let e={},o=new FormData(f),i=o.getAll("general-settings"),r=o.getAll("formats-settings");i.forEach(t=>e[t]=!0),e.formats=r,ti(e),"formats-settings"===t.target.name&&(S=await tb.create(r))}function N(t){e.torch=!e.torch,tg({el:t.currentTarget,isTorchOn:e.hasAttribute("torch")})}function B(t){if("function"!=typeof e.restartVideoStream)return;let o=t.target.value||void 0;e.restartVideoStream(o)}d.accept=G.join(","),o.supportedFormats=k,l.addEventListener("click",function(){l?.setAttribute("hidden",""),h?.removeAttribute("hidden"),T?.removeAttribute("hidden"),R()}),t.addEventListener("a-tab-show",ta(function(t){let e=t.detail.tabId,o=document.querySelector("video-capture");if("cameraTab"===e){if(A=!0,!o)return;if(!o.loading&&l.hasAttribute("hidden")&&(h?.removeAttribute("hidden"),T?.removeAttribute("hidden"),R()),"function"==typeof o.startVideoStream){let t=y?.value||void 0;o.startVideoStream(t)}}else"fileTab"===e&&(A=!1,null!=o&&"function"==typeof o.stopVideoStream&&o.stopVideoStream(),h?.setAttribute("hidden",""),T?.setAttribute("hidden",""))},250)),d.addEventListener("files-dropzone-drop",function(t){M(t.detail.acceptedFiles[0])}),c.addEventListener("resize-observer:resize",function(){tm(e.shadowRoot.querySelector("video"),h)}),g.addEventListener("click",function(){v.open=!0}),f.addEventListener("change",ta(H,500)),p.addEventListener("click",function(){b.open=!0}),document.addEventListener("visibilitychange",function(){if("cameraTab"===t.querySelector("[selected]").getAttribute("id"))if("hidden"===document.visibilityState)A=!1,null!=e&&"function"==typeof e.stopVideoStream&&e.stopVideoStream();else{A=!0;let t=document.querySelector("video-capture");if(!t)return;if(!t.loading&&l.hasAttribute("hidden")&&R(),"function"==typeof t.startVideoStream){let e=y?.value||void 0;t.startVideoStream(e)}}}),document.addEventListener("keydown",function(e){"Escape"===e.key&&function(){let e=t.querySelector("#cameraTab").hasAttribute("selected"),o=!l.hidden,i=v.hasAttribute("open"),r=b.hasAttribute("open");o&&e&&!(i||r)&&l.click()}()}),document.addEventListener("bs-history-success",function(t){let{type:e,message:o}=t.detail;"add"===e&&tv(o,{variant:"success"})}),document.addEventListener("bs-history-error",function(t){let{type:e,message:o}=t.detail;("remove"===e||"empty"===e)&&b?.hide(),tv(o,{variant:"danger"})})}();
//# sourceMappingURL=barcode-scanner-web.8b41232e.js.map
